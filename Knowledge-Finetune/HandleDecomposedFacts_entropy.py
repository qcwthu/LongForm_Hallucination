...
def average_of_max_k(lst, k):
    return sum(heapq.nlargest(k, lst)) / k
...
def getprobability(model, tokenizer, oneresponse, input_tokens_chat, input_ids_chat, curfact, curfact_original_text):
    ...
    for tokenidx in range(tokenprobswithresponse_sm.shape[0]):
        ...
        # allprobswithresponse.append(oneprobwithresponse)
        ####entropy
        oneprobwithresponse = tokenprobswithresponse_sm[tokenidx].detach().cpu()
        dist = Categorical(oneprobwithresponse)
        entropy = dist.entropy()
        allprobswithresponse.append(entropy)
    ...
    max_prob = max(allprobswithresponse)
    max_3_prob =  average_of_max_k(allprobswithresponse, 3)
    max_5_prob =  average_of_max_k(allprobswithresponse, 5)
    ...
    max_5percent_prob = average_of_max_k(allprobswithresponse, tosamplenumber_5percent)
    max_10percent_prob = average_of_max_k(allprobswithresponse, tosamplenumber_10percent)
    max_15percent_prob = average_of_max_k(allprobswithresponse, tosamplenumber_15percent)
    return [arithmetic_mean_prob, geometric_mean_prob, max_prob, max_3_prob, max_5_prob, max_5percent_prob, max_10percent_prob, max_15percent_prob]
...
def handle_decomposed_facts():
    ...
    for onekey in allfacts.keys():
        ...
        if len(allsupported) > 0:
            ...
            assert len(allsupported) == len(allsupported_original_flag) == len(allsupported_original_text)
            allsupported_flag_nozero_idx = []
            for idx in range(len(allsupported_original_flag)):
                if allsupported_original_flag[idx] == 1:
        ...
        if len(allcontradicted) > 0:
            ...
            assert len(allcontradicted) == len(allcontradicted_original_flag) == len(allcontradicted_original_text)
            allcontradicted_flag_nozero_idx = []
            for idx in range(len(allcontradicted_original_flag)):
                if allcontradicted_original_flag[idx] == 1:
        ...
        if len(allsupported) > 0 and len(allcontradicted) > 0:
            ...
            if minsize > 0:
                ...
                for idx in range(minsize):
                    ...
                    res_prob_s = getprobability(model, tokenizer, oneresponse, input_tokens_chat, input_ids_chat, onesupported, onesupported_original_text)
                    ...
                    res_prob_c = getprobability(model, tokenizer, oneresponse, input_tokens_chat, input_ids_chat, onecontradicted, onecontradicted_original_text)
    ...
    for i in range(len(allsupportedres[0])):
        ...
        minvalue = min(min(tousesupportedres), min(tousecontradictedres))
        maxvalue = max(max(tousesupportedres), max(tousecontradictedres)) + 0.1
        bins = np.linspace(minvalue, maxvalue, 21)
        width = (maxvalue - minvalue) / 20
        ...
        # axs[i//4][i%4].bar(bins, allfigres_s, color='blue', alpha=0.5, label='Correct Claim', width=width)
        axs[i//4][i%4].bar(bins, allfigres_s, color='blue', alpha=0.5, label='Factual', width=width)
        ...
        # print(max_y)
        # axs[i//4][i%4].bar(bins, allfigres_c, color='orange', alpha=0.5, label='Wrong Claim', width=width)
        axs[i//4][i%4].bar(bins, allfigres_c, color='orange', alpha=0.5, label='Hallucinated', width=width)

        leftnum = math.floor(minvalue * 10) / 10
        rightnum = math.ceil(maxvalue * 10) / 10
        axs[i//4][i%4].set_xticks(np.arange(leftnum, rightnum, (rightnum - leftnum) / 5))
    ...
    # picname = "Figures/temperature_" + str(temperature) + "_entropy_1.png"
    picname = "Figures/temperature_" + str(temperature) + "_entropy_newlegend.pdf"
    plt.savefig(picname)
    print(allsize)
